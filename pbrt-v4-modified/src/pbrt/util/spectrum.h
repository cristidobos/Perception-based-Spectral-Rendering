...
class SampledWavelengths {
  public:
    ...
    static SampledWavelengths SampleUniform(Float u, Float lambda_min = Lambda_min,
        ...
        // Sample first wavelength using _u_ by selecting a 5nm bin.
        // This line implements the requested logic:
        // 1. Calculate the number of 5nm bins in the spectral range.
        // 2. Use Lerp to scale u to that number of bins.
        // 3. Convert to an integer to get a discrete bin index.
        // 4. Multiply by 5 and add lambda_min to get the final wavelength.
        swl.lambda[0] = lambda_min + lambdaStep * (int)Lerp(u, 0.f, NumDistributionBins);
    ...
    PBRT_CPU_GPU
    static SampledWavelengths SampleFromCustom(const WavelengthSamplingPDFs *wavelengthPDFs, Float u);

    // PBRT_CPU_GPU
    // static SampledWavelengths SampleCustom(Float u, Float lambda_min = Lambda_min,
    //                                         Float lambda_max = Lambda_max) {
    //     CHECK(globalSamplingTable != nullptr);

    //     SampledWavelengths swl;
    //     for (int i = 0; i < NSpectrumSamples; ++i) {
    //         const Float *cdf = &globalSamplingTable->cdfs_flat[i * NumDistributionBins];

    //         int low = 0, high = NumDistributionBins - 1;
    //         int index = high;
    //         while (low <= high) {
    ...
    //         Float t = Float(index) / Float(NumDistributionBins - 1);
    //         swl.lambda[i] = Lerp(t, lambda_min, lambda_max);

    //         const Float *pdf_dist = &globalSamplingTable->pdfs_flat[i * NumDistributionBins];
    //         swl.pdf[i] = pdf_dist[index];
    //     }
    //     return swl;
