...
PBRT_CPU_GPU
SampledWavelengths SampledWavelengths::SampleFromCustom(const WavelengthSamplingPDFs *wavelengthPDFs, Float u) {
    CHECK(wavelengthPDFs != nullptr);

    SampledWavelengths swl;
    for (int i = 0; i < NSpectrumSamples; ++i) {
        // Access the i-th CDF distribution from the table on the GPU
        const Float *cdf = &wavelengthPDFs->cdfs_flat[i * NumDistributionBins];

        // Invert the CDF using a binary search to get a bin index
        ...
        // Convert the bin index to a physical wavelength
        Float t = Float(index) / Float(NumDistributionBins - 1);
        swl.lambda[i] = Lerp(t, Lambda_min, Lambda_max);

        // Look up the corresponding PDF value for this choice
        const Float *pdf_dist = &wavelengthPDFs->pdfs_flat[i * NumDistributionBins];
        // swl.pdf[i] = pdf_dist[index];
        // swl.pdf[i] = wavelengthPDFs->mix_pdf[index] / NSpectrumSamples;
        swl.pdf[i] = wavelengthPDFs->mix_pdf[index];
    }
    return swl;
}
