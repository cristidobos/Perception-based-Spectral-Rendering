...
Reformatting options:
  ...
  --wavelength-pdf <file>       Use wavelength sampling distributions from the given CSV file.
)",
            NSpectrumSamples);
    exit(msg.empty() ? 0 : 1);
}
...
int main(int argc, char *argv[]) {
...
#endif
            ParseArg(&iter, args.end(), "wavelength-pdf", &options.wavelengthPDFFile, onError) ||
    ...
    if (!options.useGPU && !options.wavelengthPDFFile.empty()) {
        ErrorExit("The --wavelength-pdf option is not supported on the CPU. Use --gpu.");
    }
	...
	if (!options.wavelengthPDFFile.empty()) {
        InitializeWavelengthSamplingTable(options.wavelengthPDFFile);
    }
