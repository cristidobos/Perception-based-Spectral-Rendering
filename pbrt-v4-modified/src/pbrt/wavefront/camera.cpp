    ...
    ParallelFor(
        "Generate camera rays", maxQueueSize, PBRT_CPU_GPU_LAMBDA(int pixelIndex) {
            ...
            if (wavelengthPDFs != nullptr) {
                // If the table is loaded, call our new static method
                lambda = SampledWavelengths::SampleFromCustom(wavelengthPDFs, lu);
            } else {
                // Otherwise, use the original PBRT film sampling method
